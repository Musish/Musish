...
export default class PlaylistPanel extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (playlistId) {
      ...
      const playlist = isLibrary ? await music.api.library.playlist(playlistId) : await music.api.playlist(playlistId);

      const albumLength = playlist.relationships.tracks.data.reduce((totalDuration, track) => (totalDuration + track.attributes.durationInMillis), 0);
